# Deploy Kubernetes Resources without validation
Write-Host "🚀 Deploying Kubernetes Resources (No Validation)" -ForegroundColor Cyan
...
kubectl create namespace $NAMESPACE --validate=false
...
foreach ($SERVICE in $SERVICES) {
    ...
    kubectl apply -f "kubernetes/$SERVICE/deployment.yaml" --validate=false
}
